...
describe('CustomFieldForm.vue', () => {
    let wrapper
    beforeAll(async () => {
        spyOn(apiCustomfields, 'getCustomFields').and.returnValue(Promise.resolve([nonEditableString, editableMoney, editableSelectOption, nonEditableInt]))
        ...
        wrapper = mount(CustomFieldForm, {
            mocks: globals, propsData: {
                valuesCustomFields: [nonEditableStringValue, editableMoneyValue, editableSelectOptionValue, nonEditableIntValue],
                back: '/back'
            }
        })
    ...
    it('can split up fields in editable and non-editable', async () => {
        expect(wrapper.text()).toContain('NON_EDITABLE_CUSTOM_FIELDS')
        expect(wrapper.findAll(CustomFieldFormField).length).toBe(4) // all fields rendered
    ...
    it('save with error => only editable, shows errors', async () => {
        let i = 0
        spyOn(apiCustomfields, 'setCustomFieldValue').and.callFake(() => {
            if (i ==0) {
                i++
                return Promise.resolve()
            }
            return Promise.reject({ body: { reason_phrase: 'Something bad' } })
        })
        
        await wrapper.vm.send()
        expect(apiCustomfields.setCustomFieldValue).toBeCalled()
        expect(wrapper.vm.errors[15]).toBe('Something bad')
    })
